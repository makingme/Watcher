...
import kr.uracle.ums.monit.executor.Notificater;
...
public abstract class Watcher implements Runnable{
	...
	public void run() {
		...
		while(isWorking) {
			...
			}else if(resultCode == -1) { //재기동 필요 에러 - 여기서 멈추고, 매니저에서 죽은 쓰레드 다시 살리는 방향으로 진행;
				stop();
				break;
			}
			...
			synchronized(this){
				...
				while (t_wait < idleMils){
					try {
						...
						log.info("Interrupt Call에 의한 종료");			
						break;
					}
					t_wait += 1000;
				}
